...
function roleFromLabel(label: string): ReferenceRole {
  const lower = label.toLowerCase();
  if (lower.includes("product")) return "product";
  if (lower.includes("background") || lower.includes("environment")) return "background";
...
export function AquaiStudio({ moduleKey, onBack }: { moduleKey: ModuleKey; onBack: () => void }) {
  ...
  const analyzeReference = async (role: string, file: File, referenceId: string) => {
    setReferences((prev) => ({ ...prev, [role]: prev[role] ? { ...prev[role]!, status: "analyzing", error: undefined } : prev[role] }));
    const form = new FormData();
    form.append("file", file);
    form.append("role", roleFromLabel(role));
      ...
      <section className="studio-main">
        ...
        <div className="studio-grid">
          <section className="workspace-column">
            <div className="panel">
              <div className="section-heading"><div><h2>1. References</h2><p>Upload reference asli. Asset ini menjadi visual ground truth untuk pipeline AI.</p></div><span className="upload-count">{analyzedCount}/{m.refs.length} Analyzed</span></div>
              <div className="reference-grid">
                {m.refs.map((role) => {
                  const asset = references[role];
                  return <div className="reference-card" key={role}>
                    <div className="reference-card-head"><b>{role}</b><span className={asset?.status === "analyzed" ? "status-ok" : asset?.status === "failed" ? "status-error" : asset?.status === "analyzing" ? "status-wait" : "status-wait"}>{asset?.status === "analyzed" ? "Analyzed ✓" : asset?.status === "analyzing" ? "Analyzing…" : asset?.status === "failed" ? "Failed" : "Waiting"}</span></div>
                    ...
                    {asset?.status === "failed" ? <div className="reference-error">{asset.error}</div> : <div className="reference-file">{asset ? <><b>{asset.name}</b><span>{formatSize(asset.size)} · {asset.status === "analyzed" ? "Reference DNA ready" : asset.status === "analyzing" ? "Vision Analysis sedang berjalan" : "Menunggu Vision Analysis"}</span></> : "Belum ada file"}</div>}
                    ...
                    <div className="reference-actions">{asset?.status === "analyzed" && <span className="ground-truth-badge">● Ground Truth Locked</span>}{asset && <label className="replace-button" htmlFor={`replace-${role}`}>Replace<input id={`replace-${role}`} type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => onFileChange(role, e)} /></label>}</div>
                  </div>;
                })}
              </div>
