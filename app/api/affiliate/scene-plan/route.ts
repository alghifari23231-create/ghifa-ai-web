...
export async function POST(request: Request) {
  try {
    ...
    if (!referenceDNA?.product) return error("Product Reference DNA wajib tersedia sebelum Scene Planner dijalankan.");
...
VISUAL GROUND TRUTH:
The supplied Reference DNA is the only authority for visible character, product, and environment details. Never invent visual facts. Preserve the same character identity, product appearance, clothing, accessories, and background continuity across every scene. The product must be visibly present from Scene 1.
...
9. The imagePrompt must be ready for a native image-generation model and must produce a complete scene image using the supplied references.
